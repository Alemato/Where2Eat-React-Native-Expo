import {sAppToken, sFormRicercaCosa, sFormRicercaDove} from '../selectors';
import Api from '../api/Api';
import {Alert} from 'react-native';
import {logout} from '../reducers/AppReducer';
import {erroreInternoApp, erroreInternoServer} from './actionConstants';
import {GET_HOME, GET_RISTORANTE, RICERCA} from './action-types';
import {
  addRistoranti,
  addRisultatiRicerca,
...
export function serverSearch() {
  return function(dispatch, getState) {
    const state = getState();
    const token = sAppToken(state);
    const cosa = sFormRicercaCosa(state);
    const dove = sFormRicercaDove(state);
    ...
    dispatch({
      ...
      payload: Api.ricerca(
        ...
        function(disp) {
          ...
          dispatch(setEffettuazioneRicerca(true));
          dispatch(addRisultatiRicerca({ristoranti: response.data}));
        },
    ).catch(function(error) {
      console.log('error');
      if (error.response) {
